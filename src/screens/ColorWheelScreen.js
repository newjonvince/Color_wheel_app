...
function ColorWheelScreen({ navigation, currentUser, onSaveColorMatch, onLogout }) {
  ...
  useEffect(() => {
    if (route?.params?.palette?.length) {
      setPalette(route.params.palette);
    ...
    <GestureHandlerRootView style={{ flex: 1 }}>
      ...
      {useAdvancedMode ? (
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'space-evenly', marginHorizontal: 18, marginTop: 10 }}>
            <TouchableOpacity onPress={openCamera} style={{ alignItems: 'center' }}>
              <MaterialIcons name="photo-camera" size={26} color="#111" />
              <Text style={{ fontSize: 12, marginTop: 6 }}>Camera</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={openGallery} style={{ alignItems: 'center' }}>
              <Feather name="image" size={26} color="#111" />
              <Text style={{ fontSize: 12, marginTop: 6 }}>Gallery</Text>
            </TouchableOpacity>
          </View>
      ...
      {isColorLocked && (
        ...
        >
          ...
          {/* Drop-in Extractor Modal */}
          <Modal visible={showExtractor} animationType="slide" onRequestClose={() => setShowExtractor(false)}>
            <CoolorsColorExtractor
              navigation={navigation}
              navigateOnActions={false}
              initialImageUri={selectedImageUri}
              initialSlots={5}
              onComplete={({ slots, dominant }) => {
                setPalette(slots);
                setBaseHex(dominant || slots[0]);
